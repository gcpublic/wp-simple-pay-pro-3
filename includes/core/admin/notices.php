...
class Notices {
	...
	public function ssl_error() {
		...
		if ( ! is_ssl() && ( ! $this->check_if_dismissed( 'ssl' ) ) ) {
			...
			$notice_message = apply_filters( 'simpay_ssl_admin_notice_message', __( 'SSL (HTTPS) is not enabled. You will not be able to process live Stripe transactions until SSL is enabled.', 'simple-pay' ) );
	...
	public function stripe_connect_notice() {
		...
		if ( simpay_check_keys_exist() && ! simpay_get_account_id() ) {

			$notice_message = sprintf( __( 'WP Simple Pay now supports Stripe Connect for easier setup and improved security. <a href="%s">Click here</a> to connect your Stripe account.', 'simple-pay' ), admin_url( 'admin.php?page=simpay_settings&tab=keys' ) );

			self::print_notice( $notice_message, 'info' );
		}
	}
		...
		<div class="notice notice-<?php echo esc_attr( $notice_type ); ?>">
			...
			if ( ! empty( $dismiss_id ) ) {
				...
				<p>
					...
					) ); ?>"><?php _e( 'Dismiss Notice', 'simple-pay' ); ?></a>
				</p>
			<?php } ?>
		</div>
		<?php
