...
class General extends Admin_Page {
	...
	public function add_sections() {
		...
		return apply_filters( 'simpay_add_' . $this->option_group . '_' . $this->id . '_sections', array(
			...
			'general_misc'     => array(
				'title' => esc_html__( 'Misc', 'simple-pay' ),
			),
		) );
	}
				...
				if ( 'general' == $section ) {
					...
					$fields[ $section ] = array(
						...
						'locale'       => array(
							...
							'description' => esc_html__( "Specify auto-detect to display Stripe Checkout in the user's preferred language, if available. English will be used by default.", 'simple-pay' ) . '<br/><a href="https://stripe.com/docs/checkout#supported-languages" target="_blank">' . esc_html__( 'Stripe Checkout supported languages', 'simple-pay' ) . '</a>',
						),
					);
					...
					$fields[ $section ] = array(
						'payment_button_style'  => array(
							'title'   => esc_html__( 'Payment Button Style', 'simple-pay' ),
							'type'    => 'radio',
					...
					$fields[ $section ] = array(
						'beta_opt_in' => array(
							...
							'type'        => 'checkbox',
							'name'        => 'simpay_' . $this->option_group . '_' . $this->id . '[' . $section . '][beta_opt_in]',
							'id'          => 'simpay-' . $this->option_group . '-' . $this->id . '-' . $section . '-beta-opt-in',
							'value'       => $this->get_option_value( $section, 'beta_opt_in' ),
							'default'     => 'no',
	...
	public function get_locales() {
		...
		return array(
			''     => esc_html__( 'English (en) (default)', 'simple-pay' ),
			'auto' => esc_html__( 'Auto-detect locale', 'simple-pay' ),
			'zh'   => esc_html__( 'Simplified Chinese (zh)', 'simple-pay' ),
