	...
	spAdminPro = {
		...
		enableSubscriptions: function( e ) {
			...
			if ( $( el ).hasClass( 'simpay-custom-field-recurring-amount-toggle' ) ) {
				...
				subscriptionRadios.removeProp( 'disabled' );
			}
		},
		...
		addField: function( e ) {
			...
			if ( 'email' === fieldType ) {
				if ( spFormSettings.find( '.simpay-custom-field-email' ).length > 0 ) {
					alert( spGeneral.i18n.limitEmailField );
					return;
				}
			}

			if ( 'card' === fieldType ) {
				if ( spFormSettings.find( '.simpay-custom-field-card' ).length > 0 ) {
					alert( spGeneral.i18n.limitCardField );
					return;
				}
			}

			if ( 'checkout_button' === fieldType ) {
				if ( spFormSettings.find( '.simpay-custom-field-checkout-button' ).length > 0 ) {
					alert( spGeneral.i18n.limitCheckoutButtonField );
					return;
				}
			}
